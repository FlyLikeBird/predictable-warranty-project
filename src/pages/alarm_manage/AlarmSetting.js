...
function AlarmSetting({ dispatch, user, alarm, mach }) {
  ...
  let { ruleList, ruleMachs, ruleParams, sensorTypes } = alarm;
  ...
  useEffect(() => {
    setSelectedMachs(
      ruleMachs.length
        ? ruleMachs.map((i) => i.equipmentInfo.equipmentCode)
        : [],
    );
  }, [ruleMachs]);
  const columns = [
          ...
          <div>
            <a
              ...
              onClick={() => {
                ...
                dispatch({
                  type: 'alarm/fetchRuleMachs',
                  payload: { warningRuleId: row.warningRuleId },
                });
              }}
            >
              关联设备
            ...
            <Popconfirm
              ...
              onConfirm={() => {
                new Promise((resolve, reject) => {
                  dispatch({
                    type: 'alarm/delRulesAsync',
                    payload: { ruleId: row.warningRuleId, resolve, reject },
                  });
                })
                  .then(() => {
  ...
  return (
    <div className={style['card-container']} style={{ boxShadow: 'none' }}>
      <div style={{ padding: '10px 20px 0 20px' }}>
        <Button
          type="primary"
          onClick={() => setInfo({ visible: true, forEdit: false })}
        >
          添加规则
        </Button>
      </div>
      <Table
        ...
        dataSource={ruleList}
        locale={{
          emptyText: <div style={{ margin: '1rem 0' }}>还没有设置规则</div>,
        }}
        bordered={true}
      ...
      <Modal
        ...
        onOk={() => {
          let result = [];
          let prevMachs = ruleMachs.map((i) => ({
            code: i.equipmentInfo.equipmentCode,
            id: i.equipmentWarningRuleId,
          }));
          ...
          new Promise((resolve, reject) => {
            dispatch({
              type: 'alarm/bindRuleAsync',
              payload: { resolve, reject, warningRuleId: ruleId, result },
            });
          })
            .then(() => {
