  ...
  if (chartType === 'pie') {
    seriesData.push({
      ...
      data: [
        {
          name: '电费成本',
          itemStyle: { color: '#4482ff' },
          value: eleCost.reduce((sum, cur) => {
            sum += cur;
            return sum;
          }, 0),
        },
        {
          name: '维保成本',
          itemStyle: { color: '#7348fa' },
          value: maintainCost.reduce((sum, cur) => {
            sum += cur;
            return sum;
          }, 0),
        },
      ...
      >
        <div style={{ display: 'flex' }}>
          ...
          <CustomDatePicker
            noDay
            onDispatch={(action) =>
              onDispatch({ type: 'board/fetchCostTrend', payload: action })
            }
          />
        </div>
        <Radio.Group
