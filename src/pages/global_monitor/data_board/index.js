  ...
  return (
    <div style={{ height: '100%' }}>
          ...
          {boardList.map((item) => (
            <span
              ...
              style={{
                marginRight: '1rem',
                color:
                  item.key === currentIndex ? '#1890ff' : 'rgba(0, 0, 0, 0.65)',
                padding: '4px 16px',
                background:
      ...
      <div style={{ height: '140px', overflow: 'hidden' }}>
        <DataCardList
          list={(currentBoad.dataCardList || []).filter((i) => i.a)}
      ...
      <div style={{ height: 'calc( 100% - 180px)' }}>
        <ChartCardList
          list={(currentBoad.chartCardList || []).filter((i) => i.a)}
          msg={msg}
...
export default connect(({ user, board }) => ({ user, board }))(
  DataBoardManager,
);
