...
function MachRunningManager({ dispatch, user, mach }) {
  ...
  const { list, statusList, ruleParams, bindSensors, currentPage, total } =
    mach;
  ...
  return Object.keys(currentMach).length ? (
    <MachDetail
      list={list}
      ...
      onSelect={(obj) => setCurrentMach(obj)}
    />
  ) : (
    <>
      ...
      <div style={{ height: 'calc( 100% - 90px)', paddingTop: '1rem' }}>
        <div style={{ height: 'calc( 100% - 30px)' }}>
          {list.map((item, index) => {
            let runningInfo =
              statusList.filter(
                (i) => i.equipmentCode === item.equipmentCode,
              )[0] || {};
            let config = window.g;
            ...
            return (
              <div
                ...
                onClick={() => {
                  setCurrentMach({ ...item, ...runningInfo });
                }}
                style={{
                  width: '25%',
                  height: '50%',
                  ...
                  >
                    ...
                    >
                      <div style={{ width: '33%', padding: '4px' }}>
                        <div style={{ color: '#aaaeb4', fontSize: '0.8rem' }}>
                          型号
                        </div>
                      ...
                      <div style={{ width: '42%', padding: '4px' }}>
                        ...
                        <div>
                          {runningInfo.adviceUpkeepTime
                            ? runningInfo.adviceUpkeepTime.split('T')[0]
                            : '--'}
                        </div>
                      </div>
                      <div style={{ width: '15%', padding: '4px' }}>
                        ...
                        <div style={{ color: '#fea84c' }}>
                          {runningInfo.appraise || '--'}
                        </div>
                      </div>
                    </div>
                  </div>
        ...
        <Pagination
          ...
          onChange={(page) => {
            dispatch({
              ...
              payload: { currentPage: page, pageSize: 8 },
            });
          }}
        />
      </div>
...
export default connect(({ user, mach }) => ({ user, mach }))(
  MachRunningManager,
);
