...
function CustomTabs({ currentMach }) {
  ...
  const infoList = [
    { label: '设备名称', value: currentMach.equipmentName },
    { label: '品牌', value: currentMach.equipmentBrand },
    { label: '型号', value: currentMach.equipmentModel },
    { label: '能耗', value: currentMach.load, unit: 'kw' },
    {
      label: '底座类型',
      value: currentMach.equipmentPedestal === 1 ? '硬性底座' : '柔性底座',
    },
    { label: '转轴高度', value: currentMach.equipmentPivoting, unit: 'mm' },
  ];
  return (
        ...
        {tabs.map((item) => (
          <div
            className={
              style['tabs-nav-item'] +
              ' ' +
              (item.key === currentKey ? style['selected'] : '')
            }
            key={item.key}
