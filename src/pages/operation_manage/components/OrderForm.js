...
import { Form, Input, InputNumber, Button, Select, DatePicker, Upload, Switch, Modal, message } from 'antd';
...
function OrderForm({ machList, userList, info, onDispatch, onClose }){
    ...
    return (
            ...
            <Form 
                ...
                form={form}              
                onFinish={values=>{
                    let machInfo = machList.filter(i=>i.equipmentCode === values.equipmentCode )[0];
                    ...
                    values.predictedFinishTime = values.predictedFinishTime.format('YYYY-MM-DD');
                    if ( fileList.length ) {
                        values.fileList = fileList.map(i=>i.size ? i.originFileObj : i);
                    }
                    new Promise((resolve,reject)=>{
                        onDispatch({ type:'order/addOrderAsync', payload:{ values, resolve, reject, forEdit:info.forEdit }});
                    }).then(()=>{
            ...
            >
                <Form.Item name='equipmentCode' label="关联设备" rules={[{ required: true, message:'关联设备不能为空' }]}>
                    {
                        ...
                        <Select style={{ width:'100%' }}>
                            {
                                machList.map((item)=>(
                                    <Option value={item.equipmentCode} key={item.equipmentCode}>{ item.equipmentName }</Option>
                                ))
                            }
                        </Select>
                        :
                ...
                <Form.Item name='workTicketsType' label="工单类型" rules={[{ required:true, message:'请选择工单类型' }]}>
                    <Select>
                        <Option value='0'>保养工单</Option>
                        <Option value='1'>维修工单</Option>
                    </Select>
                </Form.Item>
                <Form.Item name='predictedFinishTime' label="预计完成时间" rules={[{ required:true, message:'请选择完成日期' }]}>
                    <DatePicker style={{ width:'100%' }} locale={zhCN} disabledDate={disabledDate} allowClear={false} />
                </Form.Item>
               
                <Form.Item name='equipmentHeadId' label="指派人员" rules={[{ required:true , message:'请指派人员'}]}>
                    <Select style={{ width:'100%' }}>
                        {
                            userList.map(item=>(
                ...
                <Form.Item name='workTicketsContent' label="任务描述" rules={[{ required:true, message:'请添加一些描述'}]}>
