import { getRunningReport, getCostReport } from '../services/reportService';
...
const initialState = {
  ...
  category: [
    ...
    { title: '偏差度', unit: '', dataIndex: 'accelero/Deviation', multi: true },
    {
      title: '峰值因子',
      unit: '',
      dataIndex: 'accelero/Crestfactor',
      multi: true,
    },
  ],
  runningColumns: [],
...
export default {
  ...
  effects: {
    ...
    *fetchRunningReport(action, { put, select, call }) {
      ...
      let { data } = yield call(getRunningReport, {
        companyId,
        equipmentCode,
        beginDate: startDate.format('YYYY-MM-DD') + ' 00:00:00',
    ...
    *fetchCostReport(action, { put, select, call }) {
      ...
      let { equipmentCode, currentPage } = action.payload || {};
      currentPage = currentPage || 1;
      let { data } = yield call(getCostReport, {
        companyId,
        equipmentCode,
        timeType,
  ...
  reducers: {
    ...
    getRunningReportResult(state, { payload: { data, currentPage, total } }) {
      ...
      return {
        ...state,
        runninglist: result,
        runningColumns: dateColumn,
        currentPage,
        total,
      };
    },
    getCostReportResult(state, { payload: { data, currentPage, total } }) {
      let result = [],
        dateColumn = [];
      data.forEach((item, index) => {
        let timeList = item.costMonthList;
        if (index === 0 && timeList && timeList.length) {
          timeList.forEach((sub) => {
            dateColumn.push(sub.date);
          });
        }
      });
      return {
