...
import { getOrderTrend } from '../services/orderService';
...
const initialState = {
  ...
  fieldMaps: {
    a: {
      ...
      unit: '台',
      subTitle: [
        { title: '运行中', dataIndex: '0', unit: '' },
        { title: '停机中', dataIndex: '1', unit: '' },
      ],
    },
    b: { title: '维保中电机数' },
    c: { title: '维保超时电机数' },
    ...
    f: {
      ...
      unit: '件',
      subTitle: [
        { hasArrow: true, title: '同比', dataIndex: 'yoy', unit: '%' },
        { hasArrow: true, title: '环比', dataIndex: 'chain', unit: '%' },
      ],
    },
    g: {
      title: '今日告警',
    ...
    l: {
      ...
      unit: '元',
      subTitle: [
        { title: '电费成本', dataIndex: 'eleCost', unit: '元' },
        { title: '维保成本', dataIndex: 'maintainCost', unit: '元' },
      ],
    },
    m: {
      title: '本月成本',
...
export default {
  ...
  effects: {
    *initBoard(action, { put, select }) {
      ...
      yield put({
        type: 'fetchMachStatus',
        payload: { type: ['a'], map: ['plus'] },
      });
      // 今日告警信息
      yield put({
        type: 'fetchSumAlarm',
    ...
    *updateBoardListAsync(action, { put, select, call }) {
      ...
      let { data } = yield call(updateBoardList, {
        companyId,
        dashboardsDetail: json,
      });
    ...
    *fetchMachStatus(action, { put, call, select }) {
      ...
      if (data && data.code === 200) {
        yield put({
          type: 'getStatusDataResult',
          payload: { data: data.data, type, map },
        });
      }
    },
    *fetchSumAlarm(action, { put, call, select }) {
      ...
      let { data } = yield call(getSumAlarm, {
        companyId,
    ...
    *fetchMachCost(action, { put, call, select }) {
      ...
      let { data } = yield call(getCostTrend, {
        companyId,
        beginDate: startDate.format('YYYY-MM-DD'),
        endDate: endDate.format('YYYY-MM-DD'),
        timeType,
    ...
    *fetchWarningRank(action, { put, call, select }) {
      ...
      let { data } = yield call(getMachWarningRank, {
        companyId,
        beginDate: startDate.format('YYYY-MM-DD'),
        endDate: endDate.format('YYYY-MM-DD'),
      });
    ...
    *fetchWarningTrend(action, { put, call, select }) {
      ...
      let { data } = yield call(getMachWarningTrend, {
        companyId,
        timeType,
        beginDate: startDate.format('YYYY-MM-DD'),
        endDate: endDate.format('YYYY-MM-DD'),
    ...
    *fetchAlarmTypes(action, { select, call, put }) {
      ...
      let { startDate, endDate, timeType } = action.payload || {};
      timeType = timeType === '10' ? '2' : timeType;
      let { data } = yield call(getAlarmTrend, {
        companyId,
        timeType,
        beginDate: startDate.format('YYYY-MM-DD'),
        endDate: endDate.format('YYYY-MM-DD'),
  ...
  reducers: {
    getBoardListResult(state, { payload: { data } }) {
      return {
        ...state,
        boardList:
          data && data.dashboardsDetail
            ? JSON.parse(data.dashboardsDetail)
            : state.boardList,
      };
    },
    ...
    getMachCostResult(state, { payload: { data, type } }) {
      ...
      result[type] = {
        totalCost: eleCost + maintainCost,
        eleCost,
        maintainCost,
      };
      return { ...state, statusSourceData: result };
