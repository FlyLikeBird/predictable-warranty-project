    ...
    effects:{
        ...
        *fetchRuleList(action, { select, call, put }){
            try {
                ...
                let { data } = yield call(getRuleList, { companyId, page:currentPage, pageSize:12  });
                if ( data && data.code === 200 ){
                    yield put({type:'getRuleListResult', payload:{ data:data.data, currentPage, total:data.total }});
                }
            } catch(err) {  
                console.log(err);
            }
        ...
        *bindRuleAsync(action, { call, put, all }){
            ...
            if ( result.length ) {
                let dataArr = yield all(
                    result.map(i=>call(i.action === 'bind' ? bindRule : unbindRule, i.action === 'bind' ? { warningRuleId, equipmentCode:i.payload } : { equipmentWarningRuleId:i.payload }))
                );
        ...
        *fetchAlarmList(action, { select, call, put }){
            try {
                ...
                let { currentPage } = action.payload || {};
                currentPage = currentPage || 1;
                let params = { companyId, beginData:startDate.format('YYYY-MM-DD'), endDate:endDate.format('YYYY-MM-DD'), page:currentPage, pageSize:12 };
        ...
        *fetchAlarmPercent(action, { select, call, put }){
            ...
            if ( data && data.code === 200 ){
                yield put({ type:'getAlarmPercentResult', payload:{ data:data.data }});
            }
        },
        *fetchAlarmTrend(action, { select, call, put }){
            let { user:{ companyId, startDate, endDate, timeType }, alarm:{ optional }} = yield select();
            // 1是月维度 2是日维度 , 3是时维度
            let params = { companyId, timeType:timeType === '10' ? '2' : timeType, beginDate:startDate.format('YYYY-MM-DD'), endDate:endDate.format('YYYY-MM-DD') };
            Object.keys(optional).forEach(key=>{
                if(optional[key]){
                    params[key] = optional[key];
                }
